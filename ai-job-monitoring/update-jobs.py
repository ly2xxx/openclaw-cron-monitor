...
Config via environment variables:
  ...
  PAGE_PREFIX    - JSON page file prefix (default: "linkedin-page", reads <prefix>1.json, <prefix>2.json, ...)
...
Behavior:
  - Loads every <prefix>N.json from HERE_DIR (written by fetch-jobs.py)
  - Dedupes by jobId
...
PAGES = sorted(
    (p for p in HERE_DIR.glob(f"{PAGE_PREFIX}*.json")
     if re.fullmatch(rf"{re.escape(PAGE_PREFIX)}(\d+)\.json", p.name)),
    key=lambda p: int(p.stem[len(PAGE_PREFIX):]),
)
...
if not unique_today:
    print(f"[ERR] No jobs in {PAGE_PREFIX}*.json - refusing to record an empty scan", file=sys.stderr)
    sys.exit(1)
...
def posted_from_hint(hint, today):
    h = (hint or "").strip()
    if re.fullmatch(r"\d{4}-\d{2}-\d{2}", h):  # exact date from fetch-jobs.py
        return h
...
new_md = re.sub(
    r"> daily scan: .+",
    (
        f"> daily scan: {len(PAGES)}-page scan via LinkedIn guest endpoint "
        f"(fetch-jobs.py, no login, {len(unique_today)} unique jobs); "
...
scan_section_lines = [
    ...
    "",
    f"Scanned at {TODAY_ISO} via LinkedIn guest endpoint (jobs-guest/jobs/api/seeMoreJobPostings).",
    f"Got {len(unique_today)} unique jobs across {len(PAGES)} pages. Deduped by jobId.",
    "",
]
if new_rows:
    ...
    for r in new_rows:
        scan_section_lines.append(
            "| " + " | ".join(
                str(r[k]).replace("|", "/")
                for k in ("jobId", "role", "company", "location", "posted", "link")
            ) + " |"
        )
    scan_section_lines.append("")
